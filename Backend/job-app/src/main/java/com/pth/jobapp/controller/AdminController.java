...
public class AdminController {
    ...
    public ResponseEntity<String> updateEmployer(@RequestHeader("Authorization") String token, @RequestParam String employerId, @RequestBody AdminEmployerUpdateRequest adminEmployerUpdateRequest) {
        try {
            String email = jwtService.extractUsername(token.substring(7));
            Account employerAccount = accountService.findByUsername(email);

            if (employerAccount == null || !"admin".equals(employerAccount.getRole())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Bạn không có quyền truy cập vào đây.");
            }
            ...
            updateEmployer.setAddress(adminEmployerUpdateRequest.getAddress());
            updateEmployer.setDescription(adminEmployerUpdateRequest.getDescription());
            updateEmployer.setName(adminEmployerUpdateRequest.getName());
            account.setState(adminEmployerUpdateRequest.getState());
